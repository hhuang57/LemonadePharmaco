function [outMetric,outBalance,outT,outY] = Tenofovir_retakeDose(p,p_viral,y0_viral,OutputVar,TimeLen,missDose,retakeDose)
...
% missed Dose
for T = missDose*24:24/10:(missDose*24 + 24/10*(retakeDose-1));
    y0 = Y1(end,:);
    [t,y] = ode23s(@Tenofovir_eqns,[0 24/10],y0,options,p);
...
%retake Dose
time = (missDose+1)*24 - (missDose*24 + (24/10)*retakeDose);
y0 = Y1(end,:);
y0(6) = y0(6) + D0;
[t,y] = ode23s(@Tenofovir_eqns,[0 time],y0,options,p);
