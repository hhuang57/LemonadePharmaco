...
OutputVar = 1:5;
[outAUC0,outBalance0,outT0,outY0] = Tenofovir_multipledose(p,OutputVar,TimeLen);     
cmin0=min(outY0(end-100:end));
cmax0=max(outY0(end-100:end));
...
%     SensAbs(i) = ((auc(i)-auc0)/auc0);
    ...
    cminall(i)=cmin;
    cmaxall(i)=cmax;
    Senscmin(i) = ((cminall(i)-cmin0)/cmin0)/((p(i)-p0(i))/p0(i));
    Senscmax(i) = ((cmaxall(i)-cmax0)/cmax0)/((p(i)-p0(i))/p0(i));
end

figure;
bar(Senscmin);
ylim([-1,1]);
title('Local Sensitivity: Cmin');
figure;
bar(Senscmax);
ylim([-1,1]);
title('Local Sensitivity: Cmax');
